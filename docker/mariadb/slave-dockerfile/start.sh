...
echo /docker-entrypoint.sh --innodb_buffer-pool-size=$INNOBDB_BUFFER_POOL_SIZE --wsrep_sst_auth=wsrep_sst_auth --wsrep_node_address=${HOSTNAME} --wsrep_node_incoming_address=${HOSTNAME} --wsrep_cluster_address="gcomm://" --wsrep_node_name=${HOSTNAME} ${EXTRA_OPTIONS}
/docker-entrypoint.sh --innodb_buffer-pool-size=$INNOBDB_BUFFER_POOL_SIZE --wsrep_sst_auth=${wsrep_sst_auth} --wsrep_node_address=${HOSTNAME} --wsrep_node_incoming_address=${HOSTNAME} --wsrep_cluster_address="gcomm://" --wsrep_node_name=${HOSTNAME} ${EXTRA_OPTIONS}